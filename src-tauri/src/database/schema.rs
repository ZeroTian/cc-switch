...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    12 => {
                        log::info!("迁移数据库从 v12 到 v13（添加技能分组功能）");
                        Self::migrate_v12_to_v13(conn)?;
                        Self::set_user_version(conn, 13)?;
                    }
                    13 => {
                        log::info!("迁移数据库从 v13 到 v14（技能分组添加 app 开关）");
                        Self::migrate_v13_to_v14(conn)?;
                        Self::set_user_version(conn, 14)?;
                    }
                    14 => {
                        log::info!("迁移数据库从 v14 到 v15（添加技能分组激活快照表）");
                        Self::migrate_v14_to_v15(conn)?;
                        Self::set_user_version(conn, 15)?;
                    }
                    15 => {
                        log::info!("迁移数据库从 v15 到 v16（添加工作空间功能）");
                        Self::migrate_v15_to_v16(conn)?;
                        Self::set_user_version(conn, 16)?;
                    }
                    16 => {
                        log::info!("迁移数据库从 v16 到 v17（workspaces.path 加唯一约束）");
                        Self::migrate_v16_to_v17(conn)?;
                        Self::set_user_version(conn, 17)?;
                    }
                    17 => {
                        log::info!(
                            "迁移数据库从 v17 到 v18（移除快照表，添加 workspace_group_active）"
                        );
                    ...
                    18 => {
                        log::info!(
                            "迁移数据库从 v18 到 v19（工作空间绑定表重构，移除旧分组关联表）"
                        );
                        Self::migrate_v18_to_v19(conn)?;
                        Self::set_user_version(conn, 19)?;
                    }
                    19 => {
                        log::info!("迁移数据库从 v19 到 v20（workspaces 加 sort_index 列）");
    ...
    fn migrate_v18_to_v19(conn: &Connection) -> Result<(), AppError> {
        // 本地 main 曾将工作空间最终结构标记为 v18；若已是该结构，
        // 不得再次重建 skill_groups，只需补齐 origin/main 的迁移。
        let workspace_schema_already_current =
            Self::table_exists(conn, "workspace_skill_bindings")?
                && Self::table_exists(conn, "workspace_group_bindings")?;

        if workspace_schema_already_current {
            Self::migrate_v10_to_v11(conn)?;
            Self::migrate_v11_to_v12(conn)?;
            log::info!("v18 -> v19 迁移完成：已保留本地工作空间结构并补齐 origin 数据库能力");
            return Ok(());
        }
        ...
        .map_err(|e| AppError::Database(e.to_string()))?;
        conn.execute("UPDATE workspaces SET name = '全局' WHERE id = 'user'", [])
            .map_err(|e| AppError::Database(e.to_string()))?;
    ...
    fn migrate_v10_to_v11(conn: &Connection) -> Result<(), AppError> {
        ...
        // origin/main 路径或已补齐的本地 v18 数据库重复调用时保持幂等。
