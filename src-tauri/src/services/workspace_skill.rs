...
impl WorkspaceSkillService {
    ...
    pub fn sync_active_groups_to_workspace(db: &Arc<Database>, workspace_id: &str) -> Result<()> {
        ...
        let cleanup_failed = Self::cleanup_stale_entries(
            &target_skills_dir, &old_managed, &new_managed, &ssot_dir,
        );
    ...
    fn read_skills_manifest(skills_dir: &Path) -> Result<HashSet<String>> {
        ...
        let content =
            std::fs::read_to_string(&path).map_err(|e| anyhow!("读取清单失败: {e}"))?;
        ...
        for entry in dir.flatten() {
            ...
            if is_managed {
                ...
                if path.is_dir() && !path.join(MANAGED_MARKER).exists() {
                    log::info!(
                        "sync_workspace: 为遗留 copy 部署 {} 补写所有权标记",
                        name
                    );
