...
impl SkillService {
    ...
    pub fn disable_all_skills_with_db(db: &Arc<Database>) -> Result<()> {
        ...
        let disabled_apps = crate::app_config::SkillApps {
            claude: false,
            codex: false,
            gemini: false,
            opencode: false,
        ...
        for skill in &skills {
            for app in &all_apps {
                if let Err(e) = Self::remove_from_app(&skill.directory, app) {
                    log::warn!("disable_all_with_db: 移除 skill {} from {:?} 失败: {e}", skill.name, app);
                }
            }
            if let Err(e) = db.update_skill_apps(&skill.id, &disabled_apps) {
                log::warn!("disable_all_with_db: 更新数据库 skill {} 失败: {e}", skill.name);
            }
        }
        Ok(())
    }

    /// 按 skill_ids + app 列表启用 skill：文件系统同步 + 数据库 enabled_* 更新
    pub fn enable_skills_by_ids_for_apps_with_db(
        db: &Arc<Database>,
        ids: &[String],
        apps: &[AppType],
    ) -> Result<Vec<String>> {
        ...
        for id in ids {
            match db.get_installed_skill(id) {
                Ok(Some(skill)) => {
                    ...
                    for app in apps {
                        if let Err(e) = Self::sync_to_app_dir(&skill.directory, app) {
                            log::warn!("enable_for_apps_with_db: 同步 skill {} to {:?} 失败: {e}", skill.name, app);
                            skill_failed = true;
                        }
                    }
                    if skill_failed {
                        ...
                        // 更新数据库：按分组 app 开关设置 enabled_*
                        let new_apps = crate::app_config::SkillApps {
                            claude: apps.contains(&AppType::Claude),
                            codex: apps.contains(&AppType::Codex),
                ...
                Ok(None) => log::warn!("enable_for_apps_with_db: skill {id} 不存在，跳过"),
                Err(e) => log::warn!("enable_for_apps_with_db: 读取 skill {id} 失败: {e}"),
            }
        }
        Ok(failed)
    }
